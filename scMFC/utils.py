...
def fuse_high_order_embeddings(
    common_z: np.ndarray | torch.Tensor,
    emb1_path: str,
    emb2_path: str,
) -> torch.Tensor:

    if isinstance(common_z, np.ndarray):
        common_z = torch.tensor(common_z, dtype=torch.float32)
    else:
        common_z = common_z.to(dtype=torch.float32)

    emb1 = np.loadtxt(emb1_path, dtype=float)
    emb1 = torch.tensor(emb1, dtype=torch.float32)

    emb2 = np.loadtxt(emb2_path, dtype=float)
    emb2 = torch.tensor(emb2, dtype=torch.float32)

    fused_embeddings = torch.cat([common_z, emb1, emb2], dim=-1)
